...
module tb_kernellum_blocal_equivalence;
    ...
    kernellum_blocal_mac_array #(.ROWS(ROWS), .COLS(COLS)) candidate (
        .clk(clk), .rst(rst), .clear_acc(clear_acc), .step(step),
    ...
    task run_trial(input integer trial);
        begin
            for (r = 0; r < ROWS*COLS; r = r + 1) expected[r] = 0;
            ...
            @(negedge clk); step = 0;
            repeat (ROWS + COLS + 5) @(posedge clk); #1;
            for (r = 0; r < ROWS; r = r + 1)
                for (c = 0; c < COLS; c = c + 1) begin
                    if ($signed(full_acc[(r*COLS+c)*ACC_WIDTH +: ACC_WIDTH]) !== expected[r*COLS+c])
                        $fatal(1, "full local mismatch %0d,%0d", r, c);
                    if ($signed(candidate_acc[(r*COLS+c)*ACC_WIDTH +: ACC_WIDTH]) !== expected[r*COLS+c])
                        $fatal(1, "B-local mismatch %0d,%0d", r, c);
                end
    ...
    endtask
    initial begin
        repeat (2) @(negedge clk); rst = 0; clear_acc = 0;
        for (phase = 0; phase < 2; phase = phase + 1) run_trial(phase);
        $display("KERNELLUM_BLOCAL_EQUIVALENCE_PASS rows=%0d cols=%0d", ROWS, COLS);
        $finish;
    end
endmodule
