...
module tb_kernellum_preserved_equivalence;
    ...
    kernellum_preserved_selective_mac_array #(.ROWS(ROWS), .COLS(COLS)) candidate (
        .clk(clk), .rst(rst), .clear_acc(clear_acc), .step(step),
            ...
            for (r = 0; r < ROWS; r = r + 1)
                for (c = 0; c < COLS; c = c + 1) begin
                    ...
                    if ($signed(candidate_acc[(r*COLS+c)*ACC_WIDTH +: ACC_WIDTH]) !==
                        expected[r*COLS+c])
                        $fatal(1, "preserved-replica mismatch %0d,%0d",r,c);
                end
            @(negedge clk);
            clear_acc = 1;
            @(negedge clk);
    ...
    initial begin
        ...
        for (phase = 0; phase < 2; phase = phase + 1)
            run_trial(phase);
        $display("KERNELLUM_PRESERVED_EQUIVALENCE_PASS rows=%0d cols=%0d",
                 ROWS, COLS);
        $finish;
    end
endmodule
