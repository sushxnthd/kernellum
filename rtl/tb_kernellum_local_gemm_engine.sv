...
module tb_kernellum_local_gemm_engine;
    ...
    initial begin
        ...
        if (read_cell(0,0) !== 58 || read_cell(0,1) !== 64 ||
            ...
            $display("FAIL LOCAL GEMM: %0d %0d %0d %0d",
        ...
        if (read_cell(0,0) !== 59 || read_cell(0,1) !== 65 ||
            ...
            $display("FAIL LOCAL ACCUMULATE: %0d %0d %0d %0d",
                read_cell(0,0), read_cell(0,1), read_cell(1,0), read_cell(1,1));
            $fatal(1);
        end

        $display("KERNELLUM_K1_LOCAL_GEMM_SIM_PASS");
        $finish;
    end
endmodule
