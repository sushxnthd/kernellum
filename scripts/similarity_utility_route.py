...
OUT = ROOT / "results"
BUILD = ROOT / "build" / "similarity_utility"
OUT.mkdir(exist_ok=True)
BUILD.mkdir(parents=True, exist_ok=True)

ARCHITECTURES = (
    ("r09_c09_k192", 9, 9, 192),
    ("r09_c09_k384", 9, 9, 384),
    ("r09_c13_k192", 9, 13, 192),
    ("r09_c13_k384", 9, 13, 384),
    ("r13_c09_k192", 13, 9, 192),
    ("r13_c09_k384", 13, 9, 384),
    ("r11_c13_k192", 11, 13, 192),
    ("r11_c13_k384", 11, 13, 384),
    ("r13_c11_k192", 13, 11, 192),
    ("r13_c11_k384", 13, 11, 384),
)
...
def main() -> int:
    ...
    parser.add_argument("--seed", type=int, choices=(32, 33, 34), required=True)
    args = parser.parse_args()

    rows = []
    for name, nr, nc, kt in ARCHITECTURES:
        print(f"[utility-confirm] {name} {args.topology} seed={args.seed}", flush=True)
        row = route_one(name, nr, nc, kt, args.topology, args.seed)
        rows.append(row)
        print(
            f"[utility-confirm] ok={row['route_ok']} "
            f"fmax={row['routed_fmax_mhz']} dsp={row['synth_dsp']}",
            flush=True,
        )

    path = OUT / f"similarity_utility_{args.topology}_s{args.seed}.csv"
