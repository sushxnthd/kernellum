...
OUT = ROOT / "results"
BUILD = ROOT / "build" / "similarity_depth"
OUT.mkdir(exist_ok=True)
BUILD.mkdir(parents=True, exist_ok=True)

ARCHITECTURES = (
    ("r08_c08_k128", 8, 8, 128),
    ("r08_c08_k256", 8, 8, 256),
    ("r10_c12_k128", 10, 12, 128),
    ("r10_c12_k256", 10, 12, 256),
    ("r12_c10_k128", 12, 10, 128),
    ("r12_c10_k256", 12, 10, 256),
    ("r10_c14_k256", 10, 14, 256),
    ("r14_c10_k256", 14, 10, 256),
)
...
def main() -> int:
    ...
    parser.add_argument("--seed", type=int, choices=(29, 30, 31), required=True)
    args = parser.parse_args()

    rows = []
    for name, nr, nc, kt in ARCHITECTURES:
        print(f"[depth-codesign] {name} {args.topology} seed={args.seed}", flush=True)
        row = route_one(name, nr, nc, kt, args.topology, args.seed)
        rows.append(row)
        print(
            f"[depth-codesign] ok={row['route_ok']} "
            f"fmax={row['routed_fmax_mhz']} dsp={row['synth_dsp']}",
            flush=True,
        )

    path = OUT / f"similarity_depth_{args.topology}_s{args.seed}.csv"
