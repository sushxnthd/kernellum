#!/usr/bin/env python3
"""Apply the frozen gates to the blocal-replica physical diagnostic."""
...
def critical_path_evidence(root: Path) -> list[dict[str, object]]:
    evidence = []
    pattern = re.compile(
        r"(nangate45|sky130hd)/blocal/s(53|71|89)/"
...
def summarize(candidate: list[dict[str, str]], baseline: list[dict[str, str]],
    ...
    for platform, seed, rows, cols in sorted(expected):
        ...
        matched.append({
            ...
            "blocal_period_ns": C,
            "blocal_q": (B-C)/B,
            "blocal_retained_benefit": (B-C)/(B-L) if B > L else None,
            "blocal_vs_broadcast_density": B*Ab/(C*Ac),
            "blocal_vs_local_density": L*Al/(C*Ac),
            "local_dffs": int(local["dff_cells"]),
            "blocal_dffs": int(c_row["dff_cells"]),
            "local_area_um2": Al, "blocal_area_um2": Ac,
        })
    ...
    metrics = (
        ...
        "noop_period_ns", "blocal_period_ns", "blocal_q",
        "blocal_retained_benefit", "blocal_vs_broadcast_density",
        "blocal_vs_local_density", "local_dffs", "blocal_dffs",
        "local_area_um2", "blocal_area_um2",
    )
    points = []
    ...
    density_wins = [
        x for x in points
        if x["median_blocal_vs_broadcast_density"] > 1
        and x["median_blocal_vs_local_density"] > 1
    ]
    ...
    gates = {
        ...
        "exact_blocal_dff_counts": len(matched) == 12 and all(
            x["blocal_dffs"] == EXPECTED_DFFS[(x["rows"], x["cols"])]
            for x in matched
        ),
        "candidate_dffs_at_most_90pct_local": len(matched) == 12 and all(
            x["blocal_dffs"] <= .90*x["local_dffs"] for x in matched
        ),
        ...
        "median_raw_timing_benefit_at_least_5pct": len(points) == 4 and all(
            x["median_blocal_q"] >= .05 for x in points
        ),
        "median_retention_at_least_70pct": len(points) == 4 and all(
            x["median_blocal_retained_benefit"] >= .70 for x in points
        ),
        ...
        "target_improves_stride2_by_0p04ns": target is not None and
            target["median_blocal_period_ns"] <=
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path,
                        default=ROOT / "results" / "similarity_blocal_summary.json")
    args = parser.parse_args()
    result = summarize(
        load_csvs(args.input, "similarity_blocal_*.csv"),
        load_csvs(args.baseline, "similarity_stride2_*.csv"),
        load_csvs(args.prior_candidate, "similarity_criticalbit_*.csv"),
        any(args.input.rglob("similarity_blocal_functional_passed")),
