#!/usr/bin/env python3
"""Route one opened-data shard of the blocal-replica diagnostic."""
...
def main() -> int:
    ...
    prior.BUILD = ROOT / "build" / "similarity_blocal_physical"
    config = f"/work/asic/blocal/config_{args.platform}.mk"
    rows = []
    for row_count, col_count in GEOMETRIES:
        print(
            f"[blocal] {args.platform} seed={args.seed} "
            f"{row_count}x{col_count}", flush=True,
        )
        row = prior.route_one(
            args.platform, "opened_diagnostic", "blocal", args.seed,
            row_count, col_count, args.flow_root.resolve(),
            args.qemu.resolve(), config,
        )
        rows.append(row)
        print(
            f"[blocal] route_ok={row['route_ok']} "
            f"period={row['period_min_ns']} error={row['error_stage']}",
            flush=True,
        )

    output = ROOT / "results" / f"similarity_blocal_{args.platform}_s{args.seed}.csv"
